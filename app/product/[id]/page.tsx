...
export default function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 弹窗状态：activeQr 可以是 'wechat' | 'whatsapp' | 'ins' | null
  const [isContactModalOpen, setIsContactModalOpen] = useState(false);
  const [activeQr, setActiveQr] = useState<'wechat' | 'whatsapp' | 'ins' | null>(null);
  const [isAdding, setIsAdding] = useState(false);

  // ⚠️ 替换为你真实的三个扫码图片链接
  const WECHAT_QR = "https://images.unsplash.com/photo-1611162617474-5b21e879e113?q=80&w=400";
  ...
  const WHATSAPP_LINK = "https://wa.me/1234567890?text=Hi,%20I'm%20interested%20in%20acquiring%20a%20masterpiece."; 
  const INSTAGRAM_LINK = "https://instagram.com/your_official_account";

  const DEFAULT_TAOBAO_LINK = "https://taobao.com";
  ...
  const handleAddToWishlist = async () => {
    ...
    if (!user) {
      showToast('info', 'Please login to save to your collection.');
      router.push('/login');
      return;
    }
    ...
    if (error) {
      if (error.code === '23505') {
        showToast('info', 'Masterpiece is already in your wishlist.');
      } else {
        showToast('error', 'Failed to add to wishlist.');
      }
    } else {
      showToast('success', 'Added to your private collection wishlist.');
    }
    setIsAdding(false);
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          <div className="space-y-6">
            <div className="relative aspect-square rounded-3xl overflow-hidden bg-[#0a0b0c] border border-white/5 shadow-2xl">
              <Image src={product.images?.[activeImg] || 'https://via.placeholder.com/800'} alt={product.title} fill className="object-cover animate-in fade-in duration-500" priority />
          ...
          <div className="flex flex-col">
            ...
            <div className="space-y-6 mb-10">
              <p className="text-gray-400 leading-loose text-lg">{product.description}</p>
